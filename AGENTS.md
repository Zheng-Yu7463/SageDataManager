...
6. 官方来源同步记录一律进入 Literature；Papers 只保留实验室自产论文、投稿版本与发表成果。
7. 同步以 DOI、官方来源 ID、规范化标题和首位作者去重；重复执行更新已有记录，不创建重复文献。
8. 每篇文献必须保留生成 BibTeX 所需的结构化元数据。可从官方来源取得时，写入引用键、论文集、页码、出版社、卷期和月份；无法确认的字段留空，不推断。
9. 同步后运行相关后端测试、归档扫描和前端构建，并确认文献目录与 BibTeX 导出数量一致。
10. 最终报告新增、更新、跳过、PDF 成功、PDF 失败和 BibTeX 可导出数量。
