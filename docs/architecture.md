...
账号生命周期操作统一写入活动记录，包括初始化实例所有者、生成管理员注册链接或密码恢复链接、受邀注册、密码重置，以及账号显示名称和启停状态更新。活动描述保留操作人和目标账号等审计上下文，但不会记录一次性令牌或密码；没有实际字段变化的更新不会产生空活动。密码重置和停用账号会递增账号的会话代次，使此前签发的浏览器会话立即失效。
