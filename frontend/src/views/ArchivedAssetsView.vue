...
async function restore(asset: AssetSummary) {
  if (restoringIds.value.has(asset.id)) return
...
<template>
  <div class="page archived-page">
    ...
    <div v-else-if="loadError && !assets.length" class="state-panel state-panel--error" role="alert"><CircleAlert :size="28" /><strong>无法读取归档资产</strong><p>{{ loadError }}</p><button class="button button--outline" @click="load">重试</button></div>
