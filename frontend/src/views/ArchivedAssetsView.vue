...
<template>
  <div class="page archived-page">
    <header class="page-heading">
      <div><p class="eyebrow">SAGE RETENTION</p><h1>已归档资产</h1><p>归档资产不会出现在普通目录中，原始文件和元数据均被保留，可随时恢复。</p></div>
      <button class="button button--outline" :disabled="loading" @click="load"><RefreshCw :size="16" />刷新</button>
    </header>
    <div v-if="loading" class="state-panel"><span class="loader-ring"></span><p>正在读取归档资产…</p></div>
    <div v-else-if="error && !assets.length" class="state-panel state-panel--error"><CircleAlert :size="28" /><strong>无法读取归档资产</strong><p>{{ error }}</p><button class="button button--outline" @click="load">重试</button></div>
