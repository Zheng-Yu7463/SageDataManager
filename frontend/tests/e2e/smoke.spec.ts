...
test('AI 访问令牌保护一次性明文并归档失效记录', async ({ page }) => {
  ...
  const tokenName = createDialog.getByLabel('令牌名称')
  const submitToken = createDialog.getByRole('button', { name: '创建令牌' })
  await tokenName.fill('单')
  await expect(submitToken).toBeDisabled()
  await tokenName.fill('a'.repeat(100))
  await expect(submitToken).toBeEnabled()
