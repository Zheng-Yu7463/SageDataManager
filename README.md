...
空数据库首次启动时，公开状态接口只报告实例是否已初始化，网页据此显示首个管理员引导。初始化请求会再次确认数据库中没有任何用户，并创建唯一的实例所有者；数据库唯一约束保证多进程并发请求也只能成功一次。实例中已有任何用户时，引导永久关闭，不开放公开注册。后续管理员只能由已登录管理员在“系统设置”中创建，每个账号使用独立密码；账号名应与服务器 SSH 用户名一致。管理员忘记密码时，可由另一位已登录管理员在同一页面重置，新密码保存后旧密码立即失效。
